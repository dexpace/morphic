...
Scoped and unscoped keys cannot collide, because every scope segment is a source keyword or index
and every vendor-extension name begins with `x-`. The first `x-` segment is therefore where the
scope ends and the name begins, so no two (scope, name) pairs spell one key — and the same gap
separates them from the non-extension entries a shared carrier holds under those scopes, such as
`openapi:encoding/<part>/allowReserved`.
