...
- **An inline `allOf` branch declaring more than the merge consumes** — a branch written inline
  ...
  (GitHub #143). A boolean branch declares no keywords at all, so there is no residue to derive;
  it is a decision about the composed node instead, stated with the `false`-schema rule above.
