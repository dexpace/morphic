...
func compileSpec(specPath string, opts compileOptions, stdout, stderr io.Writer) int {
	...
	diagCode := exitCodeFor(res.Diagnostics, opts.failOn)
	if opts.explain != "" {
	...
	if err := writeCompiled(opts.outPath, stdout, res.Document); err != nil {
		emitf(stderr, "morphic: %v\n", err)
		return writeFailureExit(diagCode)
	}
	return diagCode
}

// writeFailureExit returns the exit code for a run whose diagnostics earned
// diagCode and whose output then could not be written.
//
// A failed write does not overwrite a non-zero diagCode. Whether a destination
// can be written at all is a property of the destination — /dev/null and a
// read-only directory both refuse the temp file replaceFile publishes through —
// not of the spec, so letting it decide the exit code made "the spec reached the
// --fail-on threshold" report 1 or 2 depending on where -o pointed. The verdict
// on the spec is the same either way and the write failure is on stderr either
// way, so the exit code keeps the verdict, and 2 is left to mean a run that
// failed for a reason outside the spec.
func writeFailureExit(diagCode int) int {
	if diagCode != 0 {
		return diagCode
	}
	return 2
