...
func scalarValue(node *yaml.Node) (ir.Value, error) {
	...
	case "!!str":
		...
		// ("123") stays a string. This classifies by wire spelling alone and is
		// intentionally independent of any surrounding schema type — Value is a
		// separate channel from the type (the Type-vs-Value split), so an
		// unquoted number is recorded as a number regardless of the field's type.
		if node.Style == 0 {
