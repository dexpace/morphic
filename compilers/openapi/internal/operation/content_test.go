...
`),
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			msg := diagMessageAt(t, diags, diag.DegradedConstruct, ir.SeverityWarning, tc.at+"/schema")
...
func electedHeader(t *testing.T, doc *ir.Document) (ir.TypeID, string, ir.Unmodeled) {
	t.Helper()
	responses := findOp(t, doc, "getX").Responses
...
`))
	...
	msg := diagMessageAt(t, diags, diag.DegradedConstruct, ir.SeverityWarning, at)
