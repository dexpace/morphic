...
// TestGolden lowers a full gRPC library service and compares its IR against a
// byte-exact golden snapshot. Regenerate it with
// `go test ./compilers/protobuf -run TestGolden -update`.
func TestGolden(t *testing.T) {
	t.Parallel()
	data, err := os.ReadFile("../../testdata/golden/protobuf/library.proto")
	require.NoError(t, err)
	doc, diags, err := protobuf.New().Compile(t.Context(),
		[]compilers.Source{{Path: "library.proto", Data: data}}, compilers.Options{})
	require.NoError(t, err)
	require.NotNil(t, doc)
	for _, d := range diags {
		assert.NotEqual(t, ir.SeverityError, d.Severity, "unexpected error diagnostic: %+v", d)
	}
	irtest.CompareGolden(t, "../../testdata/golden/protobuf/library.golden.json", doc)
}
