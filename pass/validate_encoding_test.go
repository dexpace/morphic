...
// part at all: a type that is no model, a registry entry holding a typed nil —
// which must be reported rather than dereferenced — and no target at all. Every
// key on such a content addresses nothing.
