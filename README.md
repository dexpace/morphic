...
before it had a pointer, `<path>` alone for one about the file as a whole, and the IR location bare
for one an IR pass made about the document. A spec that was refused, or that no compiler
recognized, names its file the same way: the path comes from the run, not from a document.
